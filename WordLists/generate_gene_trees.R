...
construct_trees <- function(dist_mat_dict)
	...
	words <- list.files(dist_mat_dict)
	word_trees <- list()
	...
	for (i in 1:length(words))
		...
		# Rescale negative branch lengths:
		if (sum(word_trees[[i]]$edge.length < 0))
			{
			word_trees[[i]]$edge.length <- word_trees[[i]]$edge.length + (-1)*min(word_trees[[i]]$edge.length)
			}
		}
	...
	for (i in 1:length(words))
		...
		write.nexus(word_trees[[i]], file='temp.txt', translate=FALSE) # Note there has to be one round of manual replacing of scientific notation entries.
